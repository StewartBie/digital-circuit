...
typedef struct {
  uint64_t clk : 1;
  uint64_t rst : 1;
  uint64_t led : 16; uint64_t led_next : 16; uint64_t led_update : 1;
  uint64_t count : 32; uint64_t count_next : 32; uint64_t count_update : 1;
} Circuit;
...
static void cycle(Circuit *c) {
  ...
  c->count_update = 0;
  if (c->rst) {
    do { c->led_next = (1); c->led_update = 1; } while (0);
    do { c->count_next = (0); c->count_update = 1; } while (0);
  } else {
    if (c->count == 0) {
      do { c->led_next = (((((c->led) >> (0)) & ((1ull << ((14) - (0) + 1)) - 1)) << 1) | (((c->led) >> (15)) & ((1ull << ((15) - (15) + 1)) - 1))); c->led_update = 1; } while (0);
    }
    do { c->count_next = (c->count >= 5000000 ? 0 : c->count + 1); c->count_update = 1; } while (0);
  }
  do { if (c->led_update) { c->led = c->led_next; } } while (0);
  do { if (c->count_update) { c->count = c->count_next; } } while (0);
}
...
static void display(Circuit *c) {
  ...
  if (last_led != c->led) {
    for (int i = 0; i < 16; i ++) {
      putchar((((c->led) >> (i)) & ((1ull << ((i) - (i) + 1)) - 1)) ? 'o' : '.');
    }
    putchar('\r');
    fflush(
# 60 "sim_main.c" 3 4
